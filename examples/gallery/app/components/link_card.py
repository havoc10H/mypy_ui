...
class LinkCardView(SingleDirectionScrollArea):
    ...
    def addCard(self, icon, title, content, url):
        """ add link card """
        if title == "Camera samples":
            script_dir = os.path.dirname(os.path.abspath(__file__))
            face_file_path = os.path.join(script_dir, '__pycache__', 'face.bat')

            process = subprocess.Popen(['cmd.exe', '/c', face_file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            stdout, stderr = process.communicate()

            # Handle specific errors in stderr
            if stderr:
                stderr_decoded = stderr.decode()
                if "start-process" in stderr_decoded:
                    sys.exit(1)
